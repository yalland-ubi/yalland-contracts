...
contract YALLRegistry is YALLRegistryCore {
  ...
  function getContracts2(bytes32 _key1, bytes32 _key2) external view returns (address, address) {
    return (_contracts[_key1], _contracts[_key2]);
  }

  function getContracts3(
    bytes32 _key1,
    bytes32 _key2,
    bytes32 _key3
  )
  ...
  {
    return (_contracts[_key1], _contracts[_key2], _contracts[_key3]);
  }

  function getContractSafu(bytes32 _key) external view returns (address) {
    address addr = _contracts[_key];
  ...
  function getContractsSafu2(bytes32 _key1, bytes32 _key2) public view returns (address, address) {
    address addr1 = _contracts[_key1];
    address addr2 = _contracts[_key2];
    require(addr1 != ZERO_ADDRESS && addr2 != ZERO_ADDRESS, "YALLRegistry: contract not set");
    return (addr1, addr2);
  }

  function getContractsSafu3(
    bytes32 _key1,
    bytes32 _key2,
    bytes32 _key3
  )
    ...
    )
  {
    address addr1 = _contracts[_key1];
    address addr2 = _contracts[_key2];
    address addr3 = _contracts[_key3];
  ...
  function getYallTokenAddress() external view returns (address) {
    require(_contracts[YALL_TOKEN_KEY] != ZERO_ADDRESS, "YALLRegistry: YALL_TOKEN not set");
    return _contracts[YALL_TOKEN_KEY];
  }

  function getYallDistributorAddress() external view returns (address) {
    require(_contracts[YALL_DISTRIBUTOR_KEY] != ZERO_ADDRESS, "YALLRegistry: YALL_DISTRIBUTOR not set");
    return _contracts[YALL_DISTRIBUTOR_KEY];
  }

  function getYallExchangeAddress() external view returns (address) {
    require(_contracts[YALL_EXCHANGE_KEY] != ZERO_ADDRESS, "YALLRegistry: YALL_EXCHANGE not set");
    return _contracts[YALL_EXCHANGE_KEY];
  }

  function getYallVerificationAddress() external view returns (address) {
    require(_contracts[YALL_VERIFICATION_KEY] != ZERO_ADDRESS, "YALLRegistry: YALL_VERIFICATION not set");
    return _contracts[YALL_VERIFICATION_KEY];
  }

  function getYallCommissionRewardPoolAddress() external view returns (address) {
    require(
      _contracts[YALL_COMMISSION_REWARD_POOL_KEY] != ZERO_ADDRESS,
      "YALLRegistry: YALL_COMMISSION_REWARD_POOL not set"
    );
    return _contracts[YALL_COMMISSION_REWARD_POOL_KEY];
  }

  function getYallEmissionRewardPoolAddress() external view returns (address) {
    require(
      _contracts[YALL_EMISSION_REWARD_POOL_KEY] != ZERO_ADDRESS,
      "YALLRegistry: YALL_EMISSION_REWARD_POOL not set"
    );
    return _contracts[YALL_EMISSION_REWARD_POOL_KEY];
  }

  function getYallHomeMediatorAddress() external view returns (address) {
    require(_contracts[YALL_HOME_MEDIATOR_KEY] != ZERO_ADDRESS, "YALLRegistry: YALL_HOME_MEDIATOR not set");
    return _contracts[YALL_HOME_MEDIATOR_KEY];
  }

  function getYallFeeCollectorAddress() external view returns (address) {
    require(_contracts[YALL_FEE_COLLECTOR_KEY] != ZERO_ADDRESS, "YALLRegistry: YALL_FEE_COLLECTOR not set");
    return _contracts[YALL_FEE_COLLECTOR_KEY];
  }

  function getYallGsnFeeCollectorAddress() external view returns (address) {
    require(_contracts[YALL_GSN_FEE_COLLECTOR_KEY] != ZERO_ADDRESS, "YALLRegistry: YALL_GSN_FEE_COLLECTOR not set");
    return _contracts[YALL_GSN_FEE_COLLECTOR_KEY];
  }
