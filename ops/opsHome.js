...
async function fundGSNContracts(dist, token, exchange) {
  const hub = await RelayHub.at('0xd216153c06e857cd7f72665e0af1d7d82172f494');

  await hub.depositFor(dist.address, { value: ether(0.5), from: defaultSender });
  await hub.depositFor(token.address, { value: ether(0.5), from: defaultSender });
  await hub.depositFor(exchange.address, { value: ether(0.5), from: defaultSender });
}
...
async function addWhitelistedAddress(registry, token, dist) {
  assert(await registry.hasRole(superuser, await token.FEE_MANAGER_ROLE()), 'Sender has no WL manager role');
...
async function estimateExchange(dist, token, exchange) {
  ...
  console.log(
    ...
    await exchange.contract.methods.createOrder(amount).estimateGas({ from: addr })
  );
}
