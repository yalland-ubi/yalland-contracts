...
describe('YALLToken', () => {
  ...
  beforeEach(async function () {
    ...
    await dist.addMembersBeforeGenesis([keccak256('foo'), keccak256('bar'), keccak256('bazz')], [alice, bob, charlie], {
  ...
  describe('#approve()', () => {
    ...
    it('should correctly approve with fee using GSN', async function () {
      ...
      const gsnFeeCollectorBalanceBefore = await yallToken.balanceOf(gsnFeeCollector);

      const res = await yallToken.approve(charlie, transferCoinAmount, { from: alice, useGSN: true });
      ...
      const gsnFeeCollectorBalanceAfter = await yallToken.balanceOf(gsnFeeCollector);
      ...
      assertErc20BalanceChanged(gsnFeeCollectorBalanceBefore, gsnFeeCollectorBalanceAfter, gsnFee.toString());
    });
    ...
    it('should charge on each approval which uses GSN', async function () {
      ...
      assert.equal(gsnFeeCollectorBalanceBefore, ether(0));
      ...
      assertErc20BalanceChanged(
        aliceBalanceBefore,
        aliceBalanceAfter,
        gsnFee.multipliedBy(3).multipliedBy(-1).toString()
      );
      assertErc20BalanceChanged(bobBalanceBefore, bobBalanceAfter, '0');
      assertErc20BalanceChanged(
        gsnFeeCollectorBalanceBefore,
        gsnFeeCollectorBalanceAfter,
        gsnFee.multipliedBy(3).toString()
      );
    });
  });
  ...
  describe('#transferFrom()', () => {
    describe('transfer restrictions', () => {
      it('should deny transferring if a from address is not active', async function () {
        ...
        await dist.disableMembers([alice], { from: distributorVerifier });
        await assertRevert(
          yallToken.transferFrom(alice, bob, ether(1), { from: charlie }),
          'YALLToken: The address has no YALL token transfer permission'
        );
      });
    ...
    it('should correct transfer with fee using GSN', async function () {
      ...
      let res = await yallToken.transferFrom(alice, bob, transferCoinAmount.dividedBy(2).toString(), {
        from: charlie,
        useGSN: true,
      });
      assertRelayedCall(res);
      ...
      assert.equal(await yallToken.allowance(alice, charlie), transferCoinAmount.dividedBy(2).toString());

      // 2nd transfer
      res = await yallToken.transferFrom(alice, bob, transferCoinAmount.dividedBy(4).toString(), {
        from: charlie,
        useGSN: true,
      });
      assertRelayedCall(res);
      ...
      assertErc20BalanceChanged(aliceBalanceBefore, aliceBalanaceAfter, zero.minus(transferCoinAmount).toString());
      assertErc20BalanceChanged(bobBalanceBefore, bobBalanceAfter, transferCoinAmount.toString());
      assertErc20BalanceChanged(charlieBalanceBefore, charlieBalanceAfter, zero.minus(totalFees).toString());
      assertErc20BalanceChanged(feeCollectorBalanceBefore, feeCollectorBalanceAfter, transferFee.toString());
      assertErc20BalanceChanged(
        gsnFeeCollectorBalanceBefore,
        gsnFeeCollectorBalanceAfter,
        gsnFee.multipliedBy(3).toString()
      ...
      // final values
      assert.equal(await yallToken.balanceOf(alice), baseAliceBalance.minus(transferCoinAmount).toString());
      assert.equal(await yallToken.balanceOf(bob), transferCoinAmount.toString());
      assert.equal(await yallToken.balanceOf(charlie), ten.minus(gsnFee.multipliedBy(3)).minus(transferFee).toString());
    ...
    it('should correct transfer with fee', async function () {
      ...
      assertErc20BalanceChanged(aliceBalanceBefore, aliceBalanaceAfter, transferCoinAmount.multipliedBy(-1).toString());
      // bob has paid transferFee
      assertErc20BalanceChanged(bobBalanceBefore, bobBalanceAfter, transferCoinAmount.minus(transferFee).toString());
  ...
  describe('#transfer()', () => {
    ...
    it('should correct transfer with fee using GSN', async function () {
      ...
      assertErc20BalanceChanged(
        aliceBalanaceBefore,
        aliceBalanaceAfter,
        zero.minus(transferCoinAmount).minus(totalFees).toString()
      );
      ...
      // final values
      assert.equal(await yallToken.balanceOf(alice), baseAliceBalance.minus(transferCoinAmount).minus(totalFees));
    ...
    it('should correct transfer with transfer fee only', async function () {
      ...
      assertErc20BalanceChanged(
        ...
        aliceBalanaceAfter,
        zero
          .minus(transferCoinAmount)
          .minus((transferCoinAmount * feePercent) / 100)
          .toString()
      );
      assertErc20BalanceChanged(bobBalanaceBefore, bobBalanaceAfter, transferCoinAmount.toString());
    ...
    describe('#setTransferRestrictionMode()', () => {
      ...
      describe('transfer()/transferFrom() behaviour', () => {
        ...
        describe('ONLY_MEMBERS mode', () => {
          beforeEach(async function () {
            await yallToken.mint(dan, ether(10), { from: yallMinter });
            await yallToken.setTransferRestrictionMode(TransferRestrictionsMode.ONLY_MEMBERS, {
              from: yallTokenManager,
            });
          });
        ...
        describe('ONLY_WHITELIST mode', () => {
          beforeEach(async function () {
            ...
            await yallToken.setTransferRestrictionMode(TransferRestrictionsMode.ONLY_WHITELIST, {
              from: yallTokenManager,
            });
          });
