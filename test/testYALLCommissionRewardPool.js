...
const { ether, now, increaseTime, assertRevert, assertErc20BalanceChanged } = require('@galtproject/solidity-test-chest')(web3);
...
describe.skip('YALLCommissionReward Integration tests', () => {
    ...
    beforeEach(async function() {
        ...
        ({ registry, yallToken, dist, exchange, emission, proxyAdmin, homeMediator, verification, commission } = await buildCoinDistAndExchange(web3, defaultSender, {
            distributorVerifier,
            yallMinter,
            feeManager,
            exchangeManager,
    ...
    it('should distribute commission rewards correctly', async function() {
        ...
        await commission.setSources([dist.address, yallToken.address, exchange.address], { from: commissionRewardPoolManager });
        ...
        const expectedAliceRewardP0 = expectedTotalRewardP0.multipliedBy(ether(40)).dividedBy(HUNDRED_PCT).multipliedBy(30).dividedBy(50);
        const expectedBobsRewardP0 = expectedTotalRewardP0.multipliedBy(ether(40)).dividedBy(HUNDRED_PCT).multipliedBy(20).dividedBy(50);
        ...
        assert.equal(periodChangeLog.totalDelegatorsReward, expectedTotalRewardP0.multipliedBy(ether(40)).dividedBy(HUNDRED_PCT).toString());
        assert.equal(periodChangeLog.totalVerifiersReward, expectedTotalRewardP0.multipliedBy(ether(10)).dividedBy(HUNDRED_PCT).toString());
        assert.equal(periodChangeLog.totalMembersReward, expectedTotalRewardP0.multipliedBy(ether(50)).dividedBy(HUNDRED_PCT).toString());
        ...
        assert.equal(res.totalDelegatorsReward, expectedTotalRewardP0.multipliedBy(ether(40)).dividedBy(HUNDRED_PCT).toString());
        assert.equal(res.totalVerifiersReward, expectedTotalRewardP0.multipliedBy(ether(10)).dividedBy(HUNDRED_PCT).toString());
        assert.equal(res.totalMembersReward, expectedTotalRewardP0.multipliedBy(ether(50)).dividedBy(HUNDRED_PCT).toString());
        ...
        let bobsBalanceBefore = await yallToken.balanceOf(bob);
        await commission.claimDelegatorReward(0, { from: bob });
        let bobsBalanceAfter = await yallToken.balanceOf(bob);
        // await assertErc20BalanceChanged(bobsBalanceBefore, bobsBalanceAfter, expectedBobsRewardP0.toString());

        // can't claim again
        await assertRevert(commission.claimVerifierReward({ from: v1 }), 'YALLCommissionRewardPool: Already claimed for the current period');

        // a new added verifier can't claim a reward
        await verification.addVerifiers([v4]);
        await assertRevert(commission.claimVerifierReward({ from: v4 }), 'YALLRewardClaimer: Can\'t assign rewards for the creation period');
        ...
        const expectedVerifiersRewardP1 = expectedTotalRewardP1.multipliedBy(ether(60)).dividedBy(HUNDRED_PCT).plus(p0VerifiersLeftover);
        const expectedMembersRewardP1 = expectedTotalRewardP1.multipliedBy(ether(30)).dividedBy(HUNDRED_PCT).plus(p0MembersLeftover);
        ...
        const expectedAliceRewardP1 = expectedTotalRewardP1.multipliedBy(ether(10)).dividedBy(HUNDRED_PCT).multipliedBy(30).dividedBy(50);
        const expectedBobsRewardP1 = expectedTotalRewardP1.multipliedBy(ether(10)).dividedBy(HUNDRED_PCT).multipliedBy(20).dividedBy(50);
        ...
        assert.equal(periodChangeLog.totalDelegatorsReward, expectedTotalRewardP1.multipliedBy(ether(10)).dividedBy(HUNDRED_PCT).toString());
        assert.equal(
          periodChangeLog.totalVerifiersReward,
          expectedTotalRewardP1.multipliedBy(ether(60)).dividedBy(HUNDRED_PCT).plus(periodChangeLog.previousUnclaimedVerifiersReward).toString()
        );
        assert.equal(
          periodChangeLog.totalMembersReward,
          expectedTotalRewardP1.multipliedBy(ether(30)).dividedBy(HUNDRED_PCT).plus(periodChangeLog.previousUnclaimedMembersReward).toString()
        );
        ...
        assert.equal(res.totalDelegatorsReward, expectedTotalRewardP1.multipliedBy(ether(10)).dividedBy(HUNDRED_PCT).toString());
