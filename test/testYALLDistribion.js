...
describe('YALLDistribution Integration Tests', function () {
  ...
  beforeEach(async function () {
    ({ registry, yallToken, dist, genesisTimestamp } = await buildCoinDistAndExchange(defaultSender, {
      yallMinter,
      feeManager,
      distributorVerifier,
      distributorManager,
  ...
  it('should allow single member claiming his funds', async function () {
    ...
    await assertRevert(dist.claimFunds({ from: bob, useGSN: true }), 'Contract not initiated yet');
    ...
    await assertRevert(dist.requireCanClaimFundsByAddress(bob), 'Already claimed for the current period');
  ...
  describe('activate/deactivate', () => {
    ...
    it('should deny me claiming a reward if disabled at P1 and enabled at P2', async function () {
      ...
      await assertRevert(dist.requireCanClaimFundsByAddress(bob), 'One period should be skipped after re-enabling');
      await assertRevert(dist.claimFunds({ from: bob }), 'One period should be skipped after re-enabling');
    ...
    it('should deny me claiming a reward if disabled at P1 and enabled at P3', async function () {
      ...
      await assertRevert(dist.requireCanClaimFundsByAddress(bob), 'YALLRewardClaimer: Not active participant');
      await assertRevert(dist.claimFunds({ from: bob }), 'YALLRewardClaimer: Not active participant');
  ...
  describe('When paused', () => {
    it('should deny executing contracts', async function () {
      ...
      await assertRevert(dist.requireCanClaimFundsByAddress(bob), 'ACLPausable: paused');
      await assertRevert(dist.claimFunds({ from: bob }), 'ACLPausable: paused');
      await assertRevert(
        dist.distributeEmissionPoolReward(0, bob, ether(1), { from: distributorVerifier }),
        'ACLPausable: paused'
      );
      await assertRevert(dist.changeMyAddress(alice, { from: charlie }), 'ACLPausable: paused');
    });
  });
});
