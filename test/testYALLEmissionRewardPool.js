...
const { buildCoinDistAndExchange, deployWithProxy } = require('./builders');
...
describe('YALLEmissionReward Integration tests', () => {
    ...
    beforeEach(async function() {
        ...
        ({ registry, yallToken, dist, exchange, emission, proxyAdmin, homeMediator, verification } = await buildCoinDistAndExchange(web3, defaultSender, {
            distributorVerifier,
            yallMinter,
            feeManager,
            exchangeManager,
    ...
    it('should correctly handle emission distribution', async function() {
        ...
        let blah = await emission.handlePeriodTransitionIfRequired();
        ...
        await assertRevert(emission.claimVerifierReward({ from: v1 }), 'YALLEmissionRewardPool: Already claimed for the current period');
